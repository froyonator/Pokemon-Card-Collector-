...
- Eleven unrelated Japanese and Simplified Chinese cards that merely shared the print number 011 were all showing the same sticky-note merchandise photo, and the Armored Mewtwo promo showed a movie still instead of the card. Two image lookups could accept a picture without any evidence it belonged to that exact card: one guessed a filename that collapsed to just the bare number for non-Latin card names, and the other trusted whatever page came back for a bare card name, even a page about the character rather than the card. Both now refuse anything that is not verifiably this card's own scan. Armored Mewtwo's real promo scan is restored in all six western languages; the affected Japanese and Chinese cards show the "no image available" placeholder until a genuine scan can be sourced.
...
- Binder volumes on the bookshelf home are now built as real leather-bound boxes instead of a flat cover with a strip painted on the side. Turning one on hover or keyboard focus now actually swings the spine into view with its lettering readable, rather than just squeezing a flat strip thinner. Each volume also picked up a grounded shadow that stretches with the turn, a gold-leaf look for the spine text, and a warmer, page-textured hint of the paper edge at rest.

### Fixed

- Binder volumes on the bookshelf home had shrunk to tiny floating thumbnails instead of their full book size; they now stand at their proper size again, spine label and cover art intact, with the hover turn still working as intended.
...
- Simplified Chinese showed no Pokemon at all, and Traditional Chinese, Thai, Indonesian, and Japanese were each missing a large slice of their cards. The underlying data recorded rarity as a short code (like "C" or "AR") instead of the full name the app's rarity filters actually look for, so thousands of cards matched no filter and simply never appeared anywhere, including several Chinese-exclusive full art prints. Those rarities are now translated to the names the app recognizes, so the missing cards show up again. The same problem affected French, German, Spanish, Italian, and Portuguese, where over half of each language's cards recorded rarity in that language's own word (like "Häufig" or "Comune") instead of the name the app's rarity filters look for; those are translated too, so cards in those five languages that were previously invisible now show up. As a side effect, the rarity label shown on cards in these five languages now reads in English rather than the local language; showing it in the local language again is a follow-up.
...
- Big Gen 1 database growth in the languages that needed it most: Simplified Chinese grew from 86 to 2,511 cards (a full sweep of the mainland-exclusive sets), Indonesian from 244 to 1,311, Thai from 299 to 685, Traditional Chinese picked up the seven newest Mega-era sets (1,811 to 1,890 cards), German gained a hundred cards from classic sets it was missing, and English and Japanese picked up their missing promo and prize sets. Nearly all new cards come with card images.
...
- A new "VMAX" option in the generation filter, right alongside Mega. Turning it on adds one tile per Dynamax/Gigantamax form, 81 in total, each showing genuine Gigantamax artwork where an official one exists and the regular Pokemon's own sprite otherwise. Clicking a VMAX tile opens a picker showing only that Pokemon's VMAX card prints, including the two forms of Urshifu and Calyrex split correctly onto their own Single Strike/Rapid Strike and Ice Rider/Shadow Rider tiles. A matching new "VMAX" card rarity group works exactly like the Mega one: it is purely additive, on by default, and turning on the VMAX generation by itself automatically switches your active rarity groups to just VMAX so its cards show up right away, switching back the moment you turn VMAX off or add another generation alongside it.
- Four new regional-form options in the generation filter: Alolan, Galarian, Hisuian, and Paldean, covering every official regional form and regional-exclusive evolution (Alolan Vulpix, Galarian Ponyta, Hisuian Growlithe, Paldean Tauros's three breeds, and species like Obstagoon and Clodsire that exist only as a regional form). Each gets its own tile with its own sprite where one exists. Because a regional form used to show up mixed in on its original species' own tile, that species' tile and picker no longer offer a regional-tagged card as an option, since it now lives exclusively on its own regional tile; a card you had already recorded as owned or wishlisted there beforehand is left exactly where it is, so nothing you already tracked disappears. The Mega/VMAX/regional options are grouped together behind a new "Forms" fold inside Generations so the filter panel stays compact.
...
- Around 6,800 French, German, Spanish, Italian, Portuguese, and Japanese cards that showed a "no image available" placeholder now display their card art, across every generation. The artwork of a given print is identical across languages, so these cards borrow the already-hosted English scan of the exact same card.
...
- Several Mega tiles could show an empty picker even though real cards exist for them (reported live for Mega Blastoise), because turning on the Mega generation didn't also turn on the Mega rarity group, and some Mega prints use a rarity that isn't part of any other group. Fixed by the Mega generation/rarity auto-switch described above. Also fixed: a Mega form that comes in two versions, like Mega Charizard X and Y or the newly added Mega Garchomp, Lucario, and Absol Z forms, could show its sibling's cards on the wrong tile. Each variant tile now only shows cards that name that exact variant; a card whose name doesn't call out a version still shows on every version's tile for the classic X/Y forms, matching how those older cards were actually printed, except for a handful of specific older Charizard and Mewtwo prints (like "M Charizard-EX" from Generations and BREAKthrough's two "M Mewtwo-EX" prints) that have now been individually confirmed and moved onto their one true tile instead. A Mega tile could also wrongly pick up a card that belongs to a different Mega Pokemon shown alongside it on the same team-up card (Mega Tyranitar was showing "Mega Sableye & Tyranitar-GX", which now correctly shows only on Mega Sableye), and a Mega tile could keep showing outdated cards after a fix like this one until you clicked Refresh Data; Mega tiles now always recompute from the latest data as soon as they load, with no manual refresh needed.
...
- Twenty-five newer Mega forms (Mega Floette, Mega Greninja, and Mega Pyroar among them) looked like they had no Mega artwork and showed the regular Pokemon's sprite instead. Their Mega art was there all along but was being requested with the wrong file ending. Every one of the 96 Mega forms now shows genuine Mega artwork, 82 of them animated; the rest stay as still art until animated versions exist anywhere.
