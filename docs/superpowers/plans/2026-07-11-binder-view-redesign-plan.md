...
- [ ] **Step 2: Fix anything found, with regression tests, following the same TDD pattern as every task above.**
- [ ] **Step 3: Final full-suite run**: `npm test -- --run`, `npm run typecheck`, `npm run lint` — all clean.
- [ ] **Step 4: Push**: `git push` (only after explicit confirmation this is wanted, per this session's standing practice of verifying before every push).
