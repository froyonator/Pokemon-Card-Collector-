...
**Resolution of Step 2 (recorded here to close the gap):** re-tested live in the browser by temporarily setting `height: auto` on `.dexGridPanel`, `.panel`, and `.binder` via devtools -- the `.page` element's own measured box collapsed from a healthy 1048x817 down to 1048x32 (rows/gaps/padding with no real height to grow into, the exact near-zero degenerate collapse described in the disclosure comments), confirming the mechanism is still load-bearing. Restoring the three `height: 100%` rules brought it back to 1048x817. Decision: **keep the mechanism as-is, not restructured** -- it's three one-line, idiomatic CSS rules (a percentage-height chain where each link opts in, which is how CSS percentage heights work in general, not a novel pattern), and it's already thoroughly disclosed at every affected site (`App.module.css`'s `.dexGridPanel`, `App.tsx`'s and `DexGrid.tsx`'s use of it, `DexGrid.module.css`'s `.panel`, and `BinderView.module.css`'s `.binder`) plus in commits `a445496` and `b6059b0`. Restructuring it into something else would risk introducing a new, less-well-understood bug for no real benefit over the current, already-verified-correct rules. This closes the audit-trail gap: the mechanism's necessity and shape are now confirmed and recorded here, rather than the code comments pointing at this Task 10 entry with nothing on the other end.

Also confirmed empirically in the same session that a `scale()` transform on `.spread` (the zoom slider/G-mode feature from Task 5) does NOT feed back into `computeSlotSize`'s `ResizeObserver`-driven measurement: applying `transform: scale(2)` to `.spread` changed `.page`'s painted `getBoundingClientRect()` width substantially (as expected, since that reflects the visual/composited box) while its layout box (`offsetWidth`, the same box family `ResizeObserver`'s `contentRect` reports) stayed effectively unchanged. No zoom-level/measured-slot-size feedback loop exists.

- [ ] **Step 3: Fix anything else found, with regression tests, following the same TDD pattern as every task above.**

**Findings from this review pass, fixed with regression tests in `src/components/BinderView.test.tsx`/`BinderView.tsx`:**
1. G-mode's click-to-exit swallowed (`stopPropagation`+`preventDefault`) EVERY click anywhere on the page while zoom mode was active, not just clicks on the binder pages -- confirmed live: clicking "Manual arrange" while zoom mode was still active exited zoom mode but the button's own click handler never ran (`aria-pressed` stayed `false`), silently eating the click and requiring a confusing second click to actually toggle it. The same would apply to the zoom slider itself and every other Binder Settings control. Fixed by scoping the swallow-and-prevent behavior to clicks that land inside `.spread` (the actual binder pages, via a new `spreadRef`) -- a click anywhere else still exits zoom mode but is left alone to do whatever it would normally do. Regression test: `zoom > clicking a nav control (not a binder page) while in zoom mode exits zoom mode but does not swallow that click`.
2. `editingSlotIndex` (the open `SlotImageEditor`'s target slot) and `zoomedCard` (the open `CardZoomOverlay`'s card) were not included in the `activeBinder.id`/`activeBinder.config`-keyed reset effect that already covers `dragFromIndex`/`selectedIndex` -- switching binders while the custom-image editor was open would leave it open, still pointing at the OLD binder's slot index, and a subsequent Save would write into the NEW active binder at that same (now-stale) index. Fixed by adding both to the same reset effect. Regression test: `BinderView manual arrange > switching the active binder while the custom-image editor is open closes it, so Save cannot write into the new binder at a stale slot index`.

The other two seam questions from this task's opening paragraph were checked and found NOT to be bugs:
- The `isManualArrangeActive`-gated blank-slot editability re-render was already covered (and passing) by the existing test `BinderView manual arrange > marking a slot empty, then editing it outside manual arrange, opens the editor and saving persists the image to that slot`, which toggles `isManualArrangeActive` from `true` to unset on the same render tree via `rerender` (no remount) and confirms the "add a custom image" affordance appears -- confirmed again live in the browser (toggle Manual arrange on, Keep empty a slot, toggle Manual arrange off without remounting, confirm the `+` affordance appears).
- The `scale()`-vs-`ResizeObserver` interaction is addressed in Step 2's resolution above.
