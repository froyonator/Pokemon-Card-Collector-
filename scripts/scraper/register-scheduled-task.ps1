...
.PARAMETER RequireACPower
  ...
  default since most scraper machines are desktops; pass this on a laptop.
...
.EXAMPLE
  .\register-scheduled-task.ps1 -TriggerType Daily -At '02:00' -Sources pkmncards,artofpkm
  Registers a daily 2am run that only re-syncs the two non-TCGdex sources.
...
try {
    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger `
        ...
        'Runs the Pokemon Card Collector data-scraper sync (run-full-sync.ps1): TCGdex/PkmnCards/Art of Pokemon snapshots, no AI involvement.' `
