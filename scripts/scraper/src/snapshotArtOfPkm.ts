...
async function main(): Promise<void> {
  ...
  const snapshotId = `artofpkm-ja-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  const staging = path.join('data', `.${snapshotId}.staging`);
  const output = path.join('data', snapshotId);
  let published = false;
  await mkdir(staging, { recursive: false });
  try {
    const allSets = parseArtOfPkmSetList(await politeHtml('https://www.artofpkm.com/cards'));
    const sets = setId ? allSets.filter((set) => set.setId === setId) : allSets;
    if (!sets.length) throw new Error(`Art of Pokémon set not found: ${setId ?? '(all)'}`);
    let cardCount = 0;
    let skippedCount = 0;
    ...
    for (const set of sets) {
      let links;
      try {
        links = parseArtOfPkmSetPage(await politeHtml(set.url), set.setId);
        ...
        // set page must not discard every other set already scraped.
      ...
      for (const link of links) {
        try {
          const parsedRecord = parseArtOfPkmDetail(await politeHtml(link.url), link.url);
          ...
          // shape) must not discard every other card already scraped in
