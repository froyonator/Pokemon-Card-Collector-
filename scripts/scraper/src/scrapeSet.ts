...
async function main() {
  ...
  if (!region || !setId || !setSlug || !languageCode) {
    console.error(
      'Usage: npm run scrape-set -- <region> <setId> <setSlug> <languageCode> [--storage-state <path>]'
    );
    console.error(
      'Example: npm run scrape-set -- id 11921 shadowy-threats id --storage-state .auth/storage-state.json'
    );
  ...
  try {
    ...
    if (failed > 0) {
      throw new Error(`Scrape incomplete: ${failed} of ${cardLinks.length} cards failed.`);
    }
