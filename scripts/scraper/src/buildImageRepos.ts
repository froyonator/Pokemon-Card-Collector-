...
const CONCURRENCY = 24; // sharp's resize is CPU-bound; this is a throughput/memory tradeoff, not a politeness limit like the live scrapers use.
...
const TCGDEX_LANGUAGE_SNAPSHOTS: Record<string, string> = {
...
async function main(): Promise<void> {
  const tcgdexSnapshots = Object.entries(TCGDEX_LANGUAGE_SNAPSHOTS).map(([lang, dir]) => ({
  ...
  // partway through.
  for (const spec of tcgdexSnapshots) {
    await stat(spec.snapshotDir);
  }

  await buildSource({
    name: 'tcgdex (all 12 covered languages, all generations)',
    repoRoot: path.join(REPOS_DIR, 'pcc-images-tcgdex'),
    snapshots: tcgdexSnapshots,
  });

  await buildSource({
    name: 'pkmncards (en, all generations)',
    repoRoot: path.join(REPOS_DIR, 'pcc-images-pkmncards'),
    snapshots: [
      {
  ...
  await buildSource({
    name: 'artofpkm (ja, all generations)',
    repoRoot: path.join(REPOS_DIR, 'pcc-images-artofpkm'),
    snapshots: [
      {
