...
import {
  isPokemonCard,
  parseEnglishFallbackDetail,
  parseEnglishFallbackSetList,
  parseEnglishFallbackSetPage,
} from './parseEnglishFallback';
...
async function main(): Promise<void> {
  ...
  const snapshotId = `english-fallback-en-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  const staging = path.join('data', `.${snapshotId}.staging`);
  const output = path.join('data', snapshotId);
  let published = false;
  await mkdir(staging, { recursive: false });
  try {
    const allSets = parseEnglishFallbackSetList(await politeHtml('https://pkmncards.com/sets/'));
    const sets = setSlug ? allSets.filter((set) => set.setSlug === setSlug) : allSets;
    if (!sets.length) throw new Error(`English fallback set not found: ${setSlug ?? '(all)'}`);
    let cardCount = 0;
    let skippedCount = 0;
    ...
    for (const set of sets) {
      let links;
      try {
        links = parseEnglishFallbackSetPage(await politeHtml(set.url));
        ...
        // already harvested in this run -- same reasoning as the per-card
        // catch below.
      ...
      for (const link of links) {
        try {
          const record = parseEnglishFallbackDetail(await politeHtml(link.url), link.url);
          ...
          if (!record.name || !record.expansionName || !record.cardNumber || !record.imageUrl) {
            throw new Error(`Invalid English fallback detail page: ${link.url}`);
          }
          ...
          // shape) must not wipe every other card already harvested in this
    ...
    const manifest = {
      ...
      source: 'English fallback source',
