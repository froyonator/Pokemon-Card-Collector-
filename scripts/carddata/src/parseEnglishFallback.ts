...
export interface EnglishFallbackSetLink {
  setSlug: string;
  name: string;
  code: string | null;
  url: string;
...
export interface EnglishFallbackCardLink {
  cardSlug: string;
  url: string;
}

export interface EnglishFallbackRecord {
...
function absoluteEnglishFallbackUrl(value: string): string {
  return new URL(value, 'https://pkmncards.com').toString();
}

export function isPokemonCard(record: EnglishFallbackRecord): boolean {
  return record.supertype === 'Pokémon';
}

export function parseEnglishFallbackSetList(html: string): EnglishFallbackSetLink[] {
  const $ = cheerio.load(html);
  const seen = new Map<string, EnglishFallbackSetLink>();
...
export function parseEnglishFallbackSetPage(html: string): EnglishFallbackCardLink[] {
  const $ = cheerio.load(html);
  const seen = new Map<string, EnglishFallbackCardLink>();
...
export function parseEnglishFallbackDetail(html: string, sourceUrl: string): EnglishFallbackRecord {
  const $ = cheerio.load(html);
  ...
  return {
    ...
    imageUrl: imageUrl ? absoluteEnglishFallbackUrl(imageUrl) : '',
  };
}
