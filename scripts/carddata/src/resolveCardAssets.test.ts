...
import type { JapaneseFallbackIndex, EnglishFallbackIndex } from './crossValidateStaticDatabase';
...
const baseEnglishFallbackRecord: EnglishFallbackRecord = {
...
function englishIndex(record: (EnglishFallbackRecord & { imageFile?: string }) | null): FallbackAssetIndexes['english'] {
  const matchIndex: EnglishFallbackIndex = {
...
const baseJapaneseFallbackRecord: JapaneseFallbackRecord = {
  sourceCardId: '11',
  name: 'Metapod',
  japaneseName: 'トランセル',
  expansionId: '10',
...
function japaneseIndex(record: (JapaneseFallbackRecord & { imageFile?: string }) | null): FallbackAssetIndexes['japanese'] {
  const matchIndex: JapaneseFallbackIndex = {
...
describe('resolveCardAssets', () => {
  ...
  it('falls back to the English fallback source hosted image when the primary source has none and a fuzzy match is found', () => {
    const record = { ...baseEnglishFallbackRecord, imageFile: 'image.png' };
  ...
  it('leaves both URLs undefined when the primary source has no image and no fallback match is found', () => {
    ...
    const fallbackIndexes: FallbackAssetIndexes = { english: englishIndex(baseEnglishFallbackRecord) };
  ...
  it('overrides a Japanese card name that looks like an untranslated English placeholder when the fallback match has a plausible native-language name', () => {
    ...
    const fallbackIndexes: FallbackAssetIndexes = { japanese: japaneseIndex(baseJapaneseFallbackRecord) };
  ...
  it('does not override the name when a fallback match exists but its own name is also an untranslated placeholder', () => {
    ...
    const implausibleJapaneseFallbackRecord: JapaneseFallbackRecord = { ...baseJapaneseFallbackRecord, japaneseName: 'Metapod' };
    const fallbackIndexes: FallbackAssetIndexes = { japanese: japaneseIndex(implausibleJapaneseFallbackRecord) };
    const resolved = resolveCardAssets(card, fallbackIndexes);
    expect(resolved.resolvedName).toBeUndefined();
  });
  ...
  it('falls back to the Japanese fallback source hosted image when the primary source has none, using the record\'s own extension', () => {
    ...
    const record = { ...baseJapaneseFallbackRecord, imageFile: 'image.png' };
  ...
  it('leaves assets and name untouched for a language neither fallback source covers, even with no primary image', () => {
    ...
    const resolved = resolveCardAssets(card, {
      english: englishIndex(baseEnglishFallbackRecord),
      japanese: japaneseIndex(baseJapaneseFallbackRecord),
    });
    expect(resolved).toEqual({});
  });
});
