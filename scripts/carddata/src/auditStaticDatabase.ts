...
// files -- deliberately re-derives dex coverage from the RAW harvested
...
async function rawDexNumbersFor(language: string): Promise<Set<number>> {
  ...
  for (const p of recordPaths) {
    ...
    // The primary source localizes the category field per language --
    // fr/de/es/it/pt all render it as "Pokémon" (accented) rather than
    // English's "Pokemon" (see primarySource.ts's own isPokemonCard for the
    // same established fact). This data pipeline itself already filters to
    // Pokemon-only before ever writing a record.json, so this check is just
    // a defensive sanity re-verification, not the real gatekeeper -- but it
    // must recognize both spellings or it wrongly discards every
    // already-valid record for exactly those five languages.
...
async function main(): Promise<void> {
  ...
  for (const language of LANGUAGES) {
    ...
    report.push(`\n=== ${language} ===`);
    report.push(
      `  dex coverage: ${builtDexNumbers.size}/151 built, ${rawDexNumbers.size}/151 available in the raw harvest`
    );
