...
import {
  ...
  type EnglishFallbackRecord,
} from './parseEnglishFallback';

describe('English fallback source parsers', () => {
  it('parses and deduplicates set and card URLs', () => {
    ...
    expect(
      parseEnglishFallbackSetPage('<a href="https://pkmncards.com/card/pikachu-fut20-001/"><img></a>')
    ).toEqual([
      {
  ...
  it('parses the semantic structure used by a card detail page', () => {
    ...
    const record = parseEnglishFallbackDetail(html, 'https://pkmncards.com/card/pikachu-fut20-001/');
  ...
  it('identifies Pokémon cards by supertype, excluding Trainer and Energy cards', () => {
    expect(isPokemonCard({ supertype: 'Pokémon' } as EnglishFallbackRecord)).toBe(true);
    expect(isPokemonCard({ supertype: 'Trainer' } as EnglishFallbackRecord)).toBe(false);
    expect(isPokemonCard({ supertype: 'Energy' } as EnglishFallbackRecord)).toBe(false);
  });
});
