...
import {
  buildJapaneseFallbackIndex,
  buildEnglishFallbackIndex,
  containsLatinLetters,
  findJapaneseFallbackMatch,
  findLatestSnapshotDir,
  findEnglishFallbackMatch,
  type JapaneseFallbackIndex as JapaneseFallbackMatchIndex,
  type EnglishFallbackIndex as EnglishFallbackMatchIndex,
...
// modeled on EnglishFallbackRecord/JapaneseFallbackRecord themselves (those
// types describe only the page-parsed fields), but genuinely present on
// every record these indexes were built from, since a snapshot never writes
// a record.json for a card whose image download failed.
...
export interface EnglishFallbackIndex {
  matchIndex: EnglishFallbackMatchIndex;
...
export interface JapaneseFallbackIndex {
  matchIndex: JapaneseFallbackMatchIndex;
}
...
function japaneseFallbackAssets(record: JapaneseFallbackRecord): { thumbUrl: string; fullUrl: string } {
  const ext = originalExtension(record as WithImageFile<JapaneseFallbackRecord>, 'webp');
...
export function resolveCardAssets(
  ...
  if (card.language === 'en' && !card.imageBase && fallbackIndexes.english) {
    const match = findEnglishFallbackMatch(fallbackIndexes.english.matchIndex, card);
    ...
    const match = findJapaneseFallbackMatch(fallbackIndexes.japanese.matchIndex, card, card.dexNumber);
...
async function loadEnglishFallbackIndex(snapshotDir: string): Promise<EnglishFallbackIndex> {
  const [matchIndex, setSlugByCardSlug] = await Promise.all([
    buildEnglishFallbackIndex(snapshotDir),
...
async function loadJapaneseFallbackIndex(snapshotDir: string): Promise<JapaneseFallbackIndex> {
  return { matchIndex: await buildJapaneseFallbackIndex(snapshotDir) };
}
...
const ENGLISH_FALLBACK_SNAPSHOT_PREFIX = 'english-fallback-en-';
const JAPANESE_FALLBACK_SNAPSHOT_PREFIX = 'japanese-fallback-ja-';
