...
function parseArguments(args: string[]): { language: string; setId?: string; delayMs: number } {
  ...
  if (!language || !SUPPORTED_LANGUAGES.has(language)) {
    throw new Error(
      `Usage: npm run snapshot-primary -- <language> [--set <setId>] [--delay-ms <ms>]`
    );
  }
...
async function main(): Promise<void> {
  ...
  const snapshotId = `primary-${language}-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  ...
  try {
    const catalog = await politeJson<PrimarySourceSetBrief[]>(primarySourceUrl(language, 'sets'));
    ...
    for (const setBrief of selectedSets) {
      ...
      let set;
      try {
        set = await politeJson<PrimarySourceSetDetail>(primarySourceUrl(language, 'sets', setBrief.id));
        ...
        // isSafePrimarySourceId's own comment for why the character-class
        // check can't be skipped even though the id already passed the
        ...
        // other set already harvested in this run -- same reasoning as the
        // per-card catch below.
        const message = error instanceof Error ? error.message : String(error);
        console.error(`  FAILED set ${setBrief.id}: ${message}`);
        failures.push({ url: primarySourceUrl(language, 'sets', setBrief.id), error: message });
        continue;
      }
      ...
      for (const brief of set.cards) {
        try {
          const card = await politeJson<PrimarySourceCardDetail>(
            primarySourceUrl(language, 'cards', brief.id)
          );
          ...
          // validatePrimarySourceCard only checks id equality, not character
          // class, so it is not a safety net for this.
            ...
            JSON.stringify(
              {
                ...
                sourceUrl: primarySourceUrl(language, 'cards', card.id),
          ...
          // harvested in this run -- see the `failures` manifest field for
          // what to re-check afterwards.
          const message = error instanceof Error ? error.message : String(error);
          console.error(`  FAILED ${brief.id}: ${message}`);
          failures.push({ url: primarySourceUrl(language, 'cards', brief.id), error: message });
        }
      }
    ...
    const manifest = {
      ...
      source: 'Primary source cards database',
