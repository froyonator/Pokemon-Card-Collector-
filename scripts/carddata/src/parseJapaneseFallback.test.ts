...
import {
  ...
  type JapaneseFallbackRecord,
} from './parseJapaneseFallback';

describe('Japanese fallback source parsers', () => {
  it('parses set and card source identities', () => {
    expect(parseJapaneseFallbackSetList('<a class="set" href="/sets/594">Premium Deck</a>')).toEqual([
  ...
  it('dedupes multiple thumbnails that share the same sourceCardId, keeping only the first', () => {
    ...
    expect(parseJapaneseFallbackSetPage(html, '458')).toEqual([
  ...
  it('parses Japanese card identity and its image from one detail page', () => {
    ...
    expect(parseJapaneseFallbackDetail(html, 'https://www.artofpkm.com/sets/594/card/1')).toEqual({
      sourceCardId: '1',
      name: 'Cherubi',
      japaneseName: 'チェリンボ',
      expansionId: '594',
  ...
  it('preserves source identity for an unnumbered card', () => {
    ...
    expect(parseJapaneseFallbackDetail(html, 'https://www.artofpkm.com/sets/592/card/19')).toMatchObject({
      sourceCardId: '19',
      cardNumber: '',
  ...
  it('falls back to the Japanese name when a card has no official English name', () => {
    ...
    expect(parseJapaneseFallbackDetail(html, 'https://www.artofpkm.com/sets/595/card/11')).toMatchObject({
      sourceCardId: '11',
      name: '',
  ...
  it('identifies a Pokemon card by the presence of pokedexNumbers', () => {
    const baseRecord: JapaneseFallbackRecord = {
      sourceCardId: '1',
      name: 'Cherubi',
      japaneseName: 'チェリンボ',
      expansionId: '594',
