# Card database data pipeline

## Running everything automatically, without an AI agent

`run-full-sync.ps1` runs all three snapshot sources below (the primary
source, the English fallback source, the Japanese fallback source) as
parallel detached processes, retries a source that fails outright (up to 3
times by default), logs everything under `data\run-logs\`, and writes one
JSON summary per run. It needs nothing but plain PowerShell -- no AI/agent
session has to be running or watching it.
...
.\run-full-sync.ps1 -Sources primary -Languages en,ja
```
...
```powershell
Get-Content "data\run-logs\english-fallback-en-<timestamp>.out.log" -Wait
```
...
.\register-scheduled-task.ps1 -Sources english-fallback,japanese-fallback # only re-sync these two
```
...
## Recommended bulk source: the primary source

The primary source publishes a multilingual database under the MIT license
and exposes the same card metadata and paired asset identifiers through its
documented API. This is the default bulk-ingestion path because it does not
depend on bypassing an access-control challenge.

Build an immutable snapshot for one set:

```sh
npm run snapshot-primary -- en --set sv03.5
```
...
metadata, its own primary-source image URL is downloaded and hashed, and the
completed run is published under `data/primary-<timestamp>/`. A root
`manifest.json` is written last as the completion marker; failed runs are
removed.

When the primary source has metadata but no scan, the record is retained with
...
The English fallback source exposes English set and card pages containing
each card's scan and structured gameplay metadata together. Build one set
with:

```sh
npm run snapshot-english-fallback -- --set pokemon-futsal-promos-2020
```
...
## Japanese fallback source

The Japanese fallback source provides Japanese set/card identities, Japanese
and English names, set numbers, illustrator and Pokédex links, and the scan
from the same card page:

```sh
npm run snapshot-japanese-fallback -- --set 594
```
...
```sh
npm run harvest-set -- id 11921 shadowy-threats id
```
...
This data pipeline can initialize its Playwright browser context from a
standard
[Playwright storage-state](https://playwright.dev/docs/auth#reuse-signed-in-state)
JSON file. The challenge or sign-in must be completed manually outside this
data pipeline; it does not solve challenges or disguise browser automation.

Supply the exported state with a command-line option:

```sh
npm run harvest-set -- id 11921 shadowy-threats id --storage-state C:\path\to\storage-state.json
```

Or set `CARD_DATA_STORAGE_STATE` before running the normal command:

```powershell
$env:CARD_DATA_STORAGE_STATE = 'C:\path\to\storage-state.json'
npm run harvest-set -- id 11921 shadowy-threats id
```
...
shared for the entire harvest so updated cookies persist between page loads.
